...
import com.example.thermal_printer_sdk.barcode.Barcode;
...
public class EscPosPrinterCommands {
    ...
    public EscPosPrinterCommands(DeviceConnection printerConnection, EscPosCharsetEncoding charsetEncoding) {
        ...
        this.charsetEncoding = charsetEncoding != null ? charsetEncoding : new EscPosCharsetEncoding("ISO-8859-1", 68);
    }
    ...
    public  EscPosPrinterCommands printTamil(){
        if (!this.printerConnection.isConnected()) {
            return this;
        }
        try {
//            this.printCharsetsEncodingCharacters(new int[] {44,68});
            byte[] textBytes = this.charsetEncoding.getName().getBytes();
            this.printerConnection.write(textBytes);
            newLine();
            this.printerConnection.write(new byte[]{0x1B, 0x74, 0x68});
            this.printerConnection.write(new byte[]{(byte)0xE0 ,(byte)0xAE ,(byte)0xA4 ,(byte)0xE0 ,(byte)0xAE ,(byte)0xAE ,(byte)0xE0 ,(byte)0xAF ,(byte)0x8D});
            this.printerConnection.send();
//        this.printerConnection.write("0x1B' + '0x74' + '0x44".getBytes());
        }catch (Exception e){
